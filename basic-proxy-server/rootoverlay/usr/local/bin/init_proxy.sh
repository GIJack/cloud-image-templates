...
main() {
  ...
  iptables_line="-A INPUT -m multiport -p tcp -s 0.0.0.0/0 --dports ${portinput} -j ACCEPT # Proxy Server(tiny proxy)"
  ip6tables_line="-A INPUT -m multiport -p tcp --dports ${portinput} -j ACCEPT # Proxy Server(tiny proxy)"
  ...
  systemctl start tinyproxy || exit_code+=1
