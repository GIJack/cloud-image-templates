...
Modified for basic-tor-image profile. Payload writes to torrc
'''
...
def write_payload(data):
    ...
    payload_file = config['torrc']
    payload_dir  = os.path.dirname(payload_file)
    if os.path.exists(payload_dir) != True:
        os.mkdirs(payload_dir,mode=0o750)
    try:
...
def main():
    ...
    submsg("Adding payload to torrc")
    WARNS += write_payload(data)
    
    submsg("Writing donefile")
    WARNS += write_done()
    
    submsg("Restarting TOR Daemon")
    tor_exit_code = subprocess.check_call(["systemctl", "restart", "tor"])
    if tor_exit_code != 0:
        warn("systemctl restart tor threw a code")
        WARNS += 1
